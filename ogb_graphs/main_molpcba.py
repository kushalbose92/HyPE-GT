...
def train_val_pipeline(MODEL_NAME, dataset, params, net_params, dirs):
    ...
    try:
        with tqdm(range(params['epochs'])) as t:
            for epoch in t:
                ...
                epoch_train_loss, epoch_train_ap, optimizer = train_epoch(model, optimizer, device, train_loader, epoch, params["batch_accumulation"], net_params)

                epoch_val_loss, epoch_val_ap = evaluate_network(model, device, val_loader, epoch, net_params)
                _, epoch_test_ap = evaluate_network(model, device, test_loader, epoch, net_params)
    ...
    for _ in range(10):
        _, test_auc = evaluate_network(model, device, test_loader_new, epoch, net_params)
        _, train_auc = evaluate_network(model, device, train_loader, epoch, net_params)
