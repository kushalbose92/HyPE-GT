...
from train.train_ppa import train_epoch, evaluate_network 
...
"""
from nets.ppa.load_net import gnn_model 
from data.data import LoadData 
from data.ppa import GraphDataset
...
def train_val_pipeline(MODEL_NAME, dataset, params, net_params, dirs):
    ...
    if net_params['lap_pos_enc']:
        ...
        dataset.train = dataset._add_laplacian_positional_encodings(net_params['pos_enc_dim'], dataset.train)
        dataset.val = dataset._add_laplacian_positional_encodings(net_params['pos_enc_dim'], dataset.val)
        dataset.test = dataset._add_laplacian_positional_encodings(net_params['pos_enc_dim'], dataset.test)
    ...
    try:
        with tqdm(range(params['epochs'])) as t:
            for epoch in t:
                ...
                epoch_train_loss, epoch_train_acc, optimizer = train_epoch(model, optimizer, device, train_loader, net_params)
                    
                epoch_val_loss, epoch_val_acc = evaluate_network(model, device, val_loader, net_params)
                _, epoch_test_acc = evaluate_network(model, device, test_loader, net_params)        
    ...
    test_acc_list = []
    test_loader_new = DataLoader(testset, batch_size=params['batch_size'], shuffle=False)
